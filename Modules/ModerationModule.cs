...
namespace LuxuriaBot.Modules
{
    public class ModerationModule : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task PurgeAsync(uint count) 
            => await _service.PurgeAsync(Context.Channel as SocketTextChannel, count);

        [Command("kick")]
        [RequireContext(ContextType.Guild)]
        [RequireUserPermission(GuildPermission.KickMembers)]
        public async Task KickAsync(string reason = null) 
            => await _service.KickAsync(Context.Message.MentionedUsers, reason);

        [Command("ban")]
        [RequireContext(ContextType.Guild)]
        [RequireUserPermission(GuildPermission.BanMembers)]
        public async Task BanAsync(string reason = null)
            => await _service.BanAsync(Context.Message.MentionedUsers, reason);
    }
}
